import requests
import csv
from lxml import html
import re

#定义常量
TMDB_BASE_URL = "https://www.themoviedb.org"
TMDB_TOP_URL_1 = "https://www.themoviedb.org/movie/top-rated"  #高分电影榜单（第一页）
TMDB_TOP_URL_2 = "https://www.themoviedb.org/discover/movie/items" #第二页及之后的电影榜单
MOVIE_LIST_FILE = "csv_data/movie_list.csv"

#获取电影年份
def get_movie_years(movie_years):
    movie_year = movie_years[0].strip() if movie_years else ""
    return movie_year.replace("(","").replace(")","") #replace 替换

#获取电影上映时间
def get_movie_publish_date(movie_dates):
    movie_date = movie_dates[0].strip() if movie_dates else ""
    return re.search(r"(\d{4}-\d{2}-\d{2})",movie_date).group()

#获取电影时长-->分钟
def get_movie_time(movie_times):
    movie_time = movie_times[0].strip() if movie_times else ""
    hour = re.search(r"(\d+)h",movie_time)
    mi =  re.search(r"(\d+)m",movie_time)
    hour = int(hour.group(1)) if hour else 0
    mi = int(mi.group(1)) if min else 0
    return int(hour)*60 + int(mi)

#获取电影详情
def get_movie_info(movie_info_url):
    #1.发送请求，获取网页数据
    # movie_response = requests.get(movie_info_url)
    movie_response = requests.get(movie_info_url + "?language=zh-CN")
    print(f"发送请求{movie_info_url},获取电影详情数据...")

    #2.解析数据，获取电影信息
    movie_doc = html.fromstring(movie_response.text)

    movie_names = movie_doc.xpath('//*[@id="original_header"]/div[2]/section/div[1]/h2/a/text()') #名称
    movie_years = movie_doc.xpath('//*[@id="original_header"]/div[2]/section/div[1]/h2/span/text()') #年份
    movie_dates = movie_doc.xpath('//*[@id="original_header"]/div[2]/section/div[1]/div/span[2]/text()') #上映时间
    movie_types = movie_doc.xpath('//*[@id="original_header"]/div[2]/section/div[1]/div/span[3]/a/text()') # 类型
    movie_times = movie_doc.xpath('//*[@id="original_header"]/div[2]/section/div[1]/div/span[4]/text()') # 时长
    movie_grades = movie_doc.xpath('//*[@id="consensus_pill"]/div/div[1]/div/div/@data-percent') # 评分
    movie_languages = movie_doc.xpath('//*[@id="media_v4"]/div/div/div[2]/div/section/div[1]/div/section[1]/p[3]/text()') # 语言
    #movie_directors = movie_doc.xpath('//*[@id="original_header"]/div[2]/section/div[3]/ol/li[2]/p[1]/a/text()') # 导演
    movie_directors = movie_doc.xpath('//ol[@class="people no_image"]/li[@class="profile"][p[@class="character"][contains(normalize-space(),"Director")]]/p/a/text()')

    #3.返回电影信息-->字典封装
    movie_info = {
        "电影名称":movie_names[0].strip() if movie_names else "",
        "上映年份":get_movie_years(movie_years),   #movie_years[0].strip() if movie_years else ""
        "上映时间":get_movie_publish_date(movie_dates),   #movie_dates[0].strip() if movie_dates else ""
        "类型":",".join(movie_types) if movie_types else "",  #",".join()
        "时长":get_movie_time(movie_times),  #movie_times[0].strip() if movie_times else ""
        "评分":movie_grades[0].strip() if movie_grades else "",
        "语言":movie_languages[0].strip() if movie_languages else "",
        "导演":movie_directors[0].strip() if movie_directors else ""
    }
    return movie_info

#保存电影数据到csv
def save_all_movies(all_movies):
    with open(MOVIE_LIST_FILE,"w",encoding="utf-8",newline="") as f:
        writer = csv.DictWriter(f,fieldnames=["电影名称","上映年份","上映时间","类型","时长","评分","语言","导演"])
        writer.writeheader()  #写入表头
        writer.writerows(all_movies)  #写入数据

#主函数
def main():
    # 保存所有电影数据
    all_movies = []

    #循环获取电影代码1-5页
    for page_num in range(1,6):
        # 1.发送请求，获取网页数据reguests
        if page_num == 1:
            response = requests.get(TMDB_TOP_URL_1, timeout=60)
        else:
            response = requests.post(TMDB_TOP_URL_2 ,
                                    f"air_date.gte = & air_date.lte = & certification = & certification_country = SG & debug = & first_air_date.gte = & first_air_date.lte = & include_adult = false & include_softcore = false & latest_ceremony.gte = & latest_ceremony.lte = & page = {page_num} & primary_release_date.gte = & primary_release_date.lte = & region = & release_date.gte = & release_date.lte = 2027 - 02 - 11 & show_me = everything & sort_by = vote_average.desc & vote_average.gte = 0 & vote_average.lte = 10 & vote_count.gte = 300 & watch_region = SG & with_genres = & with_keywords = & with_networks = & with_origin_country = & with_original_language = & with_watch_monetization_types = & with_watch_providers = & with_release_type = & with_runtime.gte = 0 & with_runtime.lte = 400",
                                    timeout=60)
        print(f"发送请求，访问第{page_num}页数据，获取电影榜单数据")

        # 2.解析网页数据，获取电影列表xpath
        document = html.fromstring(response.text)
        movie_list = document.xpath('//div[@class="media-list-results contents"]/div')

        # 3.遍历电影列表，获取电影详情url
        for movie in movie_list:
            movie_urls = movie.xpath("./div/div/a/@href")
            if movie_urls:
                # 电影详情的url
                movie_info_url = TMDB_BASE_URL + movie_urls[0]
                # 发送请求，获取电影详情数据
                movie_info = get_movie_info(movie_info_url)
                all_movies.append(movie_info)

    #4.保存数据，保存为csv文件csv
    print("保存电影数据到csv文件...")
    save_all_movies(all_movies)

if __name__ == '__main__':
    main()